...
class SyntheticDatasetLoader:
    ...
    def generate_dataset(self, num_samples=10, cot=False, seed=42, split=None):
        ...
        if split:
            ...
            print(f"Train/val split saved: {len(train_data)} train / {len(val_data)} val")
        else:
            with open(self.output_dir / "metadata.json", "w") as f:
                json.dump(all_metadata, f, indent=2)

            print(f"Full dataset saved to: {self.output_dir / 'metadata.json'}")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--split", type=float, default=None, help="Optional train/val split ratio (e.g., 0.8 for 80% train).")
    ...
    if args.cot and not os.getenv("OPENAI_API_KEY"):
        raise EnvironmentError("🔐 OPENAI_API_KEY not found. Set it in your environment to use --cot.")
